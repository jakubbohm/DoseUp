...
- **Results end-to-end:** expected failures travel as SharedKernel union values — domain methods return `DomainResult`, handlers return `ApiResult`, mapped to ProblemDetails at the edge; the edge union never enters Domain (arch rule 21). Exceptions only for bugs/infrastructure.
