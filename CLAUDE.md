...
.NET 11 previews for service/domain/test projects (`net11.0` + `LangVersion preview`); **AppHost stays net10** · Aspire 13.4 · FastEndpoints · React + Vite + TypeScript PWA · Tailwind CSS v4 · Radix Primitives (unstyled behavior layer behind owned components) · openapi-typescript + openapi-fetch (generated TS contract client — [ADR-0001-platform-and-stack](docs/adr/0001-platform-and-stack.md)) · Neon serverless Postgres (Aspire Postgres container locally) · EF Core 11 previews + Npgsql · Ardalis.SmartEnum (domain closed value sets) · Wolverine + Azure Service Bus Basic (async seam) · Storage Queue visibility alarms (reminders) · Entra External ID · Azure Container Apps (API) + Azure Static Web Apps Free (web — different origins, so CORS + a deployed `config.json`) · Bicep + Azure Deployment Stacks (hand-authored IaC — [ADR-0004-delivery-and-process](docs/adr/0004-delivery-and-process.md)) · GitHub Actions · TUnit + Shouldly · ArchUnitNET · @playwright/test.
...
- **Results end-to-end:** expected failures travel as SharedKernel union values — domain methods return `DomainResult`, handlers return `ApiResult` (conversions live on the edge — `ApiResult.From`), mapped to ProblemDetails at the edge; the edge union never enters the domain side (arch rule 21). Exceptions only for bugs/infrastructure.
...
- **Domain rules ([conventions/domain-rules.md](docs/conventions/domain-rules.md)):** aggregates self-protect with pure, sync `CheckCanXxx()` checks (static-first for projections); handlers compose `RuleSet` for aggregated violations (`RuleViolations` → 409, `violations` PD array); affordances are pure; set rules get a DB-constraint backstop; rule checks never run in parallel on one `DbContext`.
...
- **Configuration ([conventions/configuration.md](docs/conventions/configuration.md)):** the app reads only `appsettings.json` + environment variables — no cloud SDK in the config path; `appsettings.json` declares **every** setting (it is the inventory), secrets as the literal `"<secret>"` (marker *and* startup tripwire); bind with `IOptions<T>` + `ValidateOnStart` — `IConfiguration` never injected outside Platform composition; no `appsettings.Production.json`; prod values are env vars authored in Bicep; local/test secrets live in the **AppHost's** user-secrets as Aspire parameters; nothing environment-specific or secret in a `VITE_` variable.
- UI-heavy changes get a **Claude Design mockup + handoff** before implementation ([conventions/design.md](docs/conventions/design.md) — the repo owns tokens/components; the design tool never writes to the repo). Handoff translation maps onto **existing** tokens/components — a genuinely new token is a flagged, reviewable decision, never silently invented.
