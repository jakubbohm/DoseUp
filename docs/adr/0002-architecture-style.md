...
│       │                    # Cosmos persistence, push sender, clock, …
...
└── Platform/                # composition root and cross-cutting infrastructure:
                             # Cosmos bootstrap, auth pipeline, ProblemDetails
                             # mapping, outbox dispatch, feature flags, OTel
```

### Dependency rules (the architecture-test contract)

1. `Domain` references only `SharedKernel` (never Features, Infrastructure, Platform, FastEndpoints, or Cosmos SDK).
...
- **Integration events:** the only asynchronous cross-module channel, published **after commit** via an **outbox**. Candidate design (M-phase): event document written in the same Cosmos transactional batch as the aggregate; **change feed processor** dispatches. Transport starts in-process with a broker-shaped seam.
...
- The outbox/change-feed design is novel enough to deserve its own design artifact (and possibly ADR) when the first integration event ships.
