...
**Status:** Accepted · **Date:** 2026-07-13 · **Decided by:** Jakub (interview 2026-07-13) · **Amended:** 2026-07-14 — infra-as-code model, release pipeline, migration & backup strategy; expand/contract mandate dropped · 2026-07-15 — C# format gate folded into the build: CSharpier dropped for `.editorconfig`/IDE0055 (conventions § Formatting) · 2026-07-19 — configuration model: app-facing env vars authored in Bicep (not passed along by the workflow), flag store deferred to `IConfiguration` + a bounded break-glass path, GitHub Actions *variables* rejected ([conventions/configuration.md](../conventions/configuration.md)) · 2026-07-19 — the web bundle joins the build-once artifact set; its per-environment `config.json` is deployment metadata, not part of the artifact ([ADR-0001-platform-and-stack § Hosting (web app)](0001-platform-and-stack.md))
...
- **Squash merge** with the PR title enforced as a **Conventional Commit** (title lint in CI); re-examined and confirmed 2026-07-14, and again 2026-07-18 — now enforced by the main-branch ruleset (`allowed_merge_methods: ["squash"]`), not convention alone: one change = one gate-verified, machine-readable, singly-revertable commit on main, with the WIP history still browsable in the PR. Corollary: the PR-head SHA never lands on main, so deployable artifacts are built from the merge commit (see CD).
- **Accepted cost of squash:** git cannot recognise a squash-merged branch as merged — the squash is a new SHA, so `main..branch` still lists its commits and `git branch -d` refuses (`-D` is correct). This is what the branch-off-freshest-main rule above buys back; a branch is never continued after its PR merges, or its commits replay against a stale merge-base. A stale branch does *not* delete main's newer files when merged — three-way merge keeps them, so the symptoms are noisy diffs and same-line conflicts, never silent loss of merged work (mis-diagnosed once, 2026-07-18).
...
- **The Aspire AppHost models local orchestration only.** AppHost ↔ Bicep sync is procedural, mirroring the TS-contract regen rule ([ADR-0001-platform-and-stack](0001-platform-and-stack.md)): a change touching the AppHost resource graph includes an explicit "update infra Bicep + `.bicepparam`" task (openspec config rule); the post-deploy smoke test backstops wiring drift. The contract being synced is **environment-variable names** — Aspire's `WithReference()` generates them locally, Bicep must reproduce them exactly ([conventions/configuration.md § 3](../conventions/configuration.md)).
- **Every app-facing environment variable is authored in Bicep** *(2026-07-19)* — from a `.bicepparam` value or straight from a resource reference in the same deployment. The release workflow supplies only the image tag and which param file to apply; it never reads a value off one resource to pass it to another. Rationale: the deployment already holds the symbolic references, so wiring there is atomic and stack-tracked, keeps secrets out of the runner's environment, and creates no cross-job ordering. Secrets reach the container app as Key Vault-sourced ACA secrets resolved by its managed identity and exposed as ordinary env vars — the app never holds a Key Vault client, which is what keeps the container runnable off Azure ([conventions/configuration.md](../conventions/configuration.md)).
- **GitHub Actions *variables* are not a configuration store** *(2026-07-19)* — anything Bicep consumes belongs in `.bicepparam`, versioned and reviewable; an Actions variable would be an unversioned parallel store for the same class of value. Sole carve-out: values the workflow needs *before* Bicep can run in order to authenticate (subscription id, resource group, environment name), which live on the GitHub Environment.
...
1. **Build once from the merge commit** — container images tagged with the git SHA, the EF Core **migration bundle** (self-contained linux-x64), and the **web bundle** (environment-agnostic — [ADR-0001-platform-and-stack § Hosting (web app)](0001-platform-and-stack.md)). These identical artifacts are what every environment receives; what differs per environment is `.bicepparam` values, GitHub Environment secrets, and the `config.json` the pipeline writes beside the web bundle — deployment metadata in the same sense `.bicepparam` is, never part of an artifact ([conventions/configuration.md § 8](../conventions/configuration.md)). Environments also carry the required-reviewers approval gate if a pre-prod environment ever exists.
...
**Microsoft.FeatureManagement over `IConfiguration`** *(store revised 2026-07-19 — Azure App Configuration deferred, [ADR-0001-platform-and-stack](0001-platform-and-stack.md))*. A flag is declared with its default in `appsettings.json` and overridden in production by an ACA environment variable declared in `.bicepparam` — which flips it **without a rebuild**, at infra-pipeline latency. Convention unchanged: incomplete features merge dark behind a flag; every flag introduction includes a removal task (tracked via the openspec change's tasks) so flags don't rot.

**Break-glass, bounded** *(2026-07-19)*: a kill switch that requires a green pipeline is not a kill switch when the pipeline is what's broken, so an out-of-band `az containerapp update` env-var flip is permitted **only** to disable something during an incident, and the reconciling `.bicepparam` commit is part of that incident, not a follow-up. This is the single sanctioned exception to the no-ad-hoc-CLI rule above; the deployment stack surfaces the drift until the commit lands.
...
- **Every change branches off the freshest main at creation** (`<type>/<change-id>`).
...
- **A change that introduces a setting declares it in `appsettings.json`** — the schema file is the settings inventory, so an undeclared setting is an invisible one ([conventions/configuration.md § 4](../conventions/configuration.md)); secrets are declared with the `"<secret>"` placeholder, never a value.
