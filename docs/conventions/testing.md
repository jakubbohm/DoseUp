...
**Status:** conventions **binding**; concrete APIs/snippets directional until the first real tests exist (M0 calibrates) · **Decided:** PRE-8 interview (2026-07-14/15) · **Stack:** fixed in [ADR-0003](../adr/0003-testing-stack.md) (TUnit + Shouldly · Aspire-only integration harness · ArchUnitNET · @playwright/test) — this document is about **organisation**: where tests live, what each layer is for, and the conventions that make test placement a lookup, not a judgment call.
...
- **Ports over data access for testability — already rejected** by PRE-7's anticipatory-abstraction rule.
...
- **Artifact freshness** is CI-gated by PRE-6: code → `openapi.json` → TS types, drift fails the build. This guarantees the TS client matches what the code *declares*.
...
- **No shared seed catalog for tests** — implicit shared data rots into mystery-guest tests. PRE-9's `UseSeeding` tier serves local dev UX, not the suite.
- **Integration tests take real time as-is.** `FakeTimeProvider` is process-local and the API runs in its own process. All time-travel testing (DST, recurrence, due-ness) lives at unit altitude — PRE-7's `now`-as-parameter design exists exactly for this. If a slice ever truly needs frozen time, the recorded mechanism is a config-injected fixed `TimeProvider` in Platform; deferred until a real need exists.
...
### 6.6 Assertions: Shouldly — confirmed against the TUnit-native alternative (PRE-8)
...
Cadence fixed by ADR-0003/0004 + PRE-9 (`ci.yml` gates only · E2E smoke-PR / full-nightly · coverage nightly, no PR threshold · mutation nightly-if-viable · ubuntu runners, health-probe waits + explicit timeouts). PRE-8 adds:
...
- **a11y automation (axe-core in Playwright):** belongs to the web-scaffold design (PRE-5/M1), where it's a strong candidate for the PWA-showcase bar — deferred to that decision, not rejected.
