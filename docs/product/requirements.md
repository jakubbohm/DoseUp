...
| NFR-4 | **Security:** HTTPS only; every API call authenticated (Entra External ID); per-account authorization enforced server-side and covered by tests; secrets via managed identity, never in the repo |
| NFR-5 | **Privacy:** Azure encryption at rest; automated backups; dose contents never appear in logs/telemetry (ids only); in-app "not medical advice" note |
| NFR-6 | **Cost:** fits a hobby budget at circle scale — working target ≤ €20/month (OQ-1): serverless Cosmos, minimal compute footprint |
