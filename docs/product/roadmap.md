...
- Tooling baseline: CSharpier + `.editorconfig` + strict analyzers, ESLint + Prettier, TUnit + Shouldly + ArchUnitNET + Playwright scaffolds — three per-layer test projects and the full arch-test catalog shipping vacuously green per [conventions/testing.md](../conventions/testing.md) (PRE-8), plus its §9 verification checklist *(largely landed by `c001-add-shared-kernel`: .editorconfig + CSharpier + strict analysis, three test projects, the 16-rule catalog, §9 walked — remaining for M0: analyzer packs, ESLint/Prettier, Playwright scaffold)*
