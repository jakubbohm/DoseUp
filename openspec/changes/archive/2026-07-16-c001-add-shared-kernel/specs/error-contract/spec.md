# error-contract — Delta Spec

The wire error model (domain-rules.md §1/§7 taxonomy, RFC 9457). One owner per error class; the single Platform mapper is the only producer of Result-derived error responses. Scoped to what this change makes observable: mapper behavior is unit-observable now; wire scenarios reference the endpoints this change ships. (The 403 request-class shape activates with M0's `ActiveAccount` — specced then, not here.)
