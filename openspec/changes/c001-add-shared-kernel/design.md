...
**Corrected 2026-07-15 (Jakub's review, task 7.9):** D8's single **app-level `DbContext` in Platform** contradicted ADR-0002's module-owned persistence tree — corrected in favor of module ownership: per-module contexts, schemas, and migration histories, canonical text in [ADR-0002 § Persistence is module property](../../../docs/adr/0002-architecture-style.md), deliberately not restated here. The c001 `DoseUpDbContext` + `Initial` migration stand as a **bootstrap placeholder** that proved the schema pipeline before any module existed — removed when the first module context lands (M1). Generalizing the `MigrationService` to iterate module contexts is an M1 task, gated on PRE-18 (the module registration locus).
