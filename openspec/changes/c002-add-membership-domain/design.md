...
*Amended 2026-07-20 (review, Jakub's call):* the helper takes `(assembly, namespacePrefix)` and each module context passes its own Domain namespace (anchored via `typeof(AccountId)`), so registration is **module-scoped** — a context never registers a foreign module's id types. The original whole-assembly sweep was model-inert but put cross-module type knowledge inside a foreign context — a boundary violation in knowledge even if not in mapping. The scan result is cached per assembly. `namespacePrefix` is optional **by the decided signature**: null deliberately means the whole assembly (a hatch for a future non-module use); module contexts always pass their Domain namespace.
...
*Amended 2026-07-20 (#99 folded in + review hardening):* (1) **Conversion direction inverted** — `DomainResult` and `RuleCheck.Fail` no longer know the edge; the edge union owns every inbound conversion as `ApiResult.From(...)` overloads (knowledge of the edge belongs to the edge; the inner types keep only `Fail.ToDomainResult()`, which points inward). (2) **`DomainResult` relocated** to `SharedKernel/Domain` — it is a domain building block; `SharedKernel/Results` now holds only the edge union. (3) **Rule 21 generalized** from "module Domain namespaces never reference the `ApiResult` type" to a direction rule — domain-side namespaces (module `Domain`, `SharedKernel.Domain`, `SharedKernel.Rules`) never reference the edge results namespace — deliberate-violation-proven on `SharedKernel.Domain.DomainResult` itself. (4) **Failure payloads hardened (review):** every violations-carrying case derives from the guarded, snapshotting `RuleViolationCarrier` (null/empty construction throws; list copied so caller mutation can't break the invariant); `ApiResult.Validation` guarded alike — including per-field arrays (null/empty message arrays throw; arrays are copied, since a pair-level dictionary copy alone would alias the mutable leaves — verification-pass finding). `default(DomainResult)` remains a designed crash per c001 — no catch-all arm, now unit-pinned. (5) **`Rules` placement decided 2026-07-20 (#99 bullet 4, Jakub's call):** the whole rule vocabulary (`RuleCheck`, `RuleViolation`, `RuleViolationCarrier`, `RuleSet`) merged from `SharedKernel/Rules` into `SharedKernel/Domain` — one domain-side namespace, rule 21's scope simplifying to module `Domain` + `SharedKernel.Domain`. Recorded nuance: `RuleSet` is async (set-rule composition awaits handler-issued queries), so the Domain namespace knowingly carries one `Task`-returning composition tool — the sync-by-convention rule governs aggregate and rule-check methods, not composition. (6) **Rule 21 anchors on `typeof`, not a string literal** (verification-pass finding): a rename/move of the edge union updates the prefix or breaks compilation, so the direction rule cannot rot into a vacuous pass.
...
- **None blocking implementation.** Parked, with owners: OQ-5 admission gating (vision, before M3) · permission model shape (future change; ADR-0002 amendment marks it) · d18 module registration (#39, when a second module or the first slice forces it) · how Jakub's own account first becomes admin (the permission-model change; nothing in M0 needs it) · #38's handler/endpoint half — whether handlers keep `Result`, share `DomainResult`, or get their own type (parked; judged against #54's real call sites). *(The NamingConventions-expiry follow-up, formerly parked here, resolved 2026-07-20 as #102 — see Risks.)*
