# membership-accounts Specification (delta)

> New capability ([proposal](../../proposal.md)): the Account aggregate's behavioral contract and its persisted schema. Domain-only in c002 — no HTTP surface exists yet, so rule denials are specced at the domain layer (stable violation codes in `DomainResult`); their 409 wire shape is `error-contract`'s existing requirement and activates when the first slice exposes it (#54's change).
