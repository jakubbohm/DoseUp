...
public static class ResultProblemDetailsMapper {
  ...
  public static ProblemDetails ToProblemDetails(this Result result) => result switch {
    Result.Success => throw new ArgumentException("Success carries no error to map — map failure results only.", nameof(result)),
