...
public sealed partial class SliceAnatomyTests {
  ...
  public void Rule_11_a_slices_endpoint_and_its_collaborators_share_one_namespace() {
    ...
    [
      .. typeof(DoseUp.Api.SharedKernel.Results.Result)
        .Assembly.GetTypes()
        .Where(static type =>
          typeof(BaseEndpoint).IsAssignableFrom(type)
          && !type.IsAbstract
          && FeatureNamespace().IsMatch(type.FullName ?? string.Empty)
        )
        .SelectMany(static endpointType =>
          endpointType
            ...
            .Where(parameter =>
              FeatureNamespace().Match(parameter.ParameterType.Namespace ?? string.Empty)
                is { Success: true } parameterModule
              && parameterModule.Groups["module"].Value
                == FeatureNamespace().Match(endpointType.FullName!).Groups["module"].Value
              && parameter.ParameterType.Namespace != endpointType.Namespace
            )
            .Select(parameter =>
              $"{endpointType.FullName} injects {parameter.ParameterType.FullName} from a different namespace"
            )
        ),
    ];
