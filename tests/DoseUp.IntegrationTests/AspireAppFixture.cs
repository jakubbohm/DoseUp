...
public sealed class AspireAppFixture : IAsyncInitializer, IAsyncDisposable {
  ...
  public async Task InitializeAsync() {
    ...
    TimeSpan timeout = Environment.GetEnvironmentVariable("CI") is null ? TimeSpan.FromMinutes(2) : TimeSpan.FromMinutes(5);

    IDistributedApplicationTestingBuilder appHost = await DistributedApplicationTestingBuilder.CreateAsync<Projects.DoseUp_AppHost>();
    ...
    await _app.ResourceNotifications.WaitForResourceAsync("migration", KnownResourceStates.Finished).WaitAsync(timeout);
  ...
  public HttpClient CreateAuthenticatedClient(Guid oid) {
    ...
    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", MintToken(oid, _signingKey));
    return client;
  }
  ...
  public HttpClient CreateUntrustedKeyClient(Guid oid) {
    HttpClient client = App.CreateHttpClient("api");
    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", MintToken(oid, _untrustedKey));
    return client;
  }

  public async Task<string> GetDatabaseConnectionStringAsync() =>
    await App.GetConnectionStringAsync("doseupdb") ?? throw new InvalidOperationException("The 'doseupdb' connection string is unavailable.");
