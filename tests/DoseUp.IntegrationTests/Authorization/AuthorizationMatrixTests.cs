...
public sealed class AuthorizationMatrixTests
{
  ...
  private static List<Type> Census() =>
    [
      .. typeof(DoseUp.Api.Platform.Diagnostics.IdentityEchoEndpoint)
        .Assembly.GetTypes()
        .Where(static type => typeof(BaseEndpoint).IsAssignableFrom(type) && !type.IsAbstract),
    ];
  ...
  public void Every_endpoint_in_the_census_is_classified()
  {
    ...
    [
      .. Census().Except(CLASSIFIED.Keys).Select(static type => type.FullName!),
    ];
    List<string> stale =
    [
      .. CLASSIFIED.Keys.Except(Census()).Select(static type => type.FullName!),
    ];
      ...
      switch (entry.Value.Kind)
      {
        case EndpointKind.Authenticated:
          yield return new MatrixCell(
            name,
            entry.Value.Route,
            nameof(CallerClass.Anonymous),
            "401"
          );
          yield return new MatrixCell(
            name,
            entry.Value.Route,
            nameof(CallerClass.UntrustedKey),
            "401"
          );
          yield return new MatrixCell(
            name,
            entry.Value.Route,
            nameof(CallerClass.Authenticated),
            "2xx"
          );
          break;
