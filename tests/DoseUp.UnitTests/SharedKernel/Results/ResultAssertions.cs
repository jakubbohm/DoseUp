...
public static class ResultAssertions {
  public static Result.Validation ShouldBeValidation(this Result result) =>
    result is Result.Validation validation ? validation : throw new ShouldAssertException($"Expected Validation but got: {Describe(result)}");

  public static Result.RuleViolations ShouldBeRuleViolations(this Result result) =>
    result is Result.RuleViolations violations ? violations : throw new ShouldAssertException($"Expected RuleViolations but got: {Describe(result)}");
